...
elseif (isset($emf) && isset($unf)&& isset($pwf1)){
  $sql="select * from ww_login where email='". $emf . "';";
  ...
  if(pg_num_rows($result)==0){
    ...
    $sql="insert into ww_login(uname,email, password) values ('" .
      $unf . "','" . $emf . "','" . $npwh . "');";
    pg_query($sql) or die('Query failed: ' . pg_last_error());
    echo '<p>ユーザ登録を完了しました</p>';
    $mailfr="tani@gms.gdl.jp";
    $mailsb="[phpua]ユーザ登録完了";
    $mailms="下記のとおりユーザ登録を完了しました。\n\n" .
      "   ユーザ名:" . $unf . "\n" .
      "   email:" . $emf . "\n\n" .
      "http://gms.gdl.jp/~tani/werewolf/login.html\n\n";
